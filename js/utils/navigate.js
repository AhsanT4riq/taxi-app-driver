...
// There is no map in the system, it is recommended to download one
        if (choices.length < 1) {
          return ActionSheetIOS.showActionSheetWithOptions({
            ...
            if (buttonIndex === 0) {
              Linking.openURL(`https://itunes.apple.com/in/app/google-maps-navigation-transport/id585027354?mt=8`);
            }
          });
        }
...
// There is no map in the system, it is recommended to download one
        if (choices.length < 1) {
          return Alert.alert('Select the map', 'You have not installed the map software yet.', [
            { text: 'Download Google Map', onPress: () => Linking.openURL(`https://play.google.com/store/apps/details?id=com.google.android.apps.maps&hl=en`) },
            { text: 'Cancel' }
          ]);
        }
        return Alert.alert('Select the map', 'Please select a map to open', [...choices, { text: 'Cancel' }]);
      });
  }
}
...
function navigate ({ destination, source, params = [] } = {}) {
  ...
  return openDialog({
    ...
    ios: [
      ...
      [
        'Use system Map',
        `http://maps.apple.com/?sddr=${source.latitude + ',' + source.longitude}&daddr=${destination.latitude + ',' + destination.longitude}`,
      ],
    ],
  });
}
